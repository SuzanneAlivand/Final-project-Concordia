...
const InProgress = () => {
      ...
      if (inProgress.length !== 0) {
        inProgress.map((id) => {
          fetch(`/api/game?id=${id}`)
            ...
            .then((data) => {
              fetchTracker++;
              if (!gamesInProgress.includes(data.data)) {
                gamesInProgress.push(data.data);
              }
              if (fetchTracker === inProgressLength) {
                setLoaded(true);
                setInProgressGames(gamesInProgress);
              }
            })
            .catch((error) => setError(true));
        });
  ...
  if (loaded && inProgressGames.length === 0) {
    return (
      <Wrapper style={{ textAlign: "center" }}>
