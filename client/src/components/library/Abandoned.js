...
const Backlog = () => {
  ...
  useEffect(() => {
    if (abandoned) {
      const abandonedLength = abandoned.length;
      let fetchTracker = 0;
      const gamesInAbandoned = [];
      if (abandoned.length !== 0) {
        abandoned.map((id) => {
          fetch(`/api/game?id=${id}`)
            ...
            .then((data) => {
              fetchTracker++;
              if (!gamesInAbandoned.includes(data.data)) {
                gamesInAbandoned.push(data.data);
              }
              if (fetchTracker === abandonedLength) {
                setLoaded(true);
                setAbandonedGames(gamesInAbandoned);
              }
            })
            .catch((error) => setError(true));
        });
  ...
  if (loaded && abandonedGames.length === 0) {
    return (
      <Wrapper style={{ textAlign: "center" }}>
